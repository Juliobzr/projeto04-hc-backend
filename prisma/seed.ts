...
async function main() {
  ...
  for (const pacienteData of pacientes) {
    ...
    // Criar ou atualizar perfil TEA
    if (tea) {
      const perfilExistente = await prisma.perfilTEA.findUnique({
        where: { pacienteId: paciente.id },
      })

      if (perfilExistente) {
        await prisma.perfilTEA.update({
          where: { pacienteId: paciente.id },
          data: tea,
        })
      } else {
    ...
    console.log(`✓ Paciente pronto: ${paciente.nome}`)
  }
